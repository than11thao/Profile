...
//     modalClose.addEventListener('click', () => {
//         modalViews.forEach(modalClick => {
//             modalClick.classList.remove('active-modal')
//         })
//     })
// })
/*=============== SWIPER TESTIMONIAL ===============*/
...
function navHighlighter() {
    ...
    //Now we loop thruogh sectilns to get height, top and ID values for each
    sections.forEach(current => {
        const sectionHeight = current.offsetHeight
        const sectionTop = current.offsetTop - 50,
        sectionId = current.getAttribute("id")
        /* - if ourr current scroll position enters the space where current se tion on sceen is, add .active class to
        corresponding navigation linkWork, else remove it
